...
class MediumAI extends AI {
  ...
  rankMove(move, board = this.game.board) {
    ...
    else switch(potentialCapture.gamePiece.name) {
    ...
    const alternateHistory = this.game.alternateHistory({row, column, canCapture}, move.originalCoordinates, board);
    const potentialEnemyMoves = this.game.getCapablePlayerMoves(alternateHistory);
    ...
    potentialEnemyMoves.forEach((move) => {
      ...
      switch(potentialCapture.gamePiece.name) {
        case "King": worstCase = 100;
            break;
        case "Queen": if(worstCase < 15) worstCase = 15;
            break;
        case "Knight":
        case "Bishop":
        case "Rook": 
            ...
            break;
        case "Pawn": if(worstCase < 5) worstCase = 5;
            break;
        default: if(worstCase < 0) worstCase = 0;
      }
      score-=worstCase;
    })
...
class Move {
  ...
  scoreStateAfterMove() {
    const {row, column} = this.moveInfo;
    const alternateHistory = this.tree.AI.game.alternateHistory({row, column}, this.moveInfo.originalCoordinates, this.boardBeforeMove);
    this.boardAfterMove = alternateHistory;
    const potentialEnemyMoves = this.tree.AI.game.getCapablePlayerMoves(alternateHistory);
