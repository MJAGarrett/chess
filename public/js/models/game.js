...
class Game {
   ...
   */
  buildBoard() {
    let board = new Array(8).fill(null).map((val) => new Array(8).fill(null));
...
/* DELETE THE BELOW WHEN SURE THEY ARE NOT NEEDED
// Start the Game
const enemy = AIFactory();
const theGame = new SingleplayerGame();
...
// Functions defined from here are likely candidates to move into a controller.
function createSquare(squareAttributes, gamePiece) {
  ...
  newSquare.addEventListener("click", () => {
    theGame.handleClick({ row, column });
  });
  if (gamePiece) {
    newSquare.classList.add(`has-${gamePiece.name}-${gamePiece.team}`);

    if (theGame.selectedPiece && theGame.selectedPiece.gamePiece === gamePiece)
      newSquare.classList.add("selected");
  }
  if (theGame.selectedPiece) {
    const squareIsPotentialMove = theGame.selectedPiece.validMoves.find(
      (move) => {
...
const updateView = () => {
  const board = theGame.board;
  let boardToRender = [];
  board.forEach((row) => {
    row.forEach((square) => {
      let newSquare = createSquare(square.squareAttributes, square.gamePiece);
      boardToRender.push(newSquare);
    });
  });
  game.replaceChildren(...boardToRender);
  updateTurnIndicator(theGame.whoseTurn);
  updateCheckIndicator(theGame.inCheck);
  updateCheckmateIndicator(theGame.checkmate);
};

const updateTurnIndicator = (turn) => {
  const properText = turn.charAt(0).toUpperCase() + turn.slice(1);
  document.querySelector(".turn").textContent = properText;
};
const updateCheckIndicator = (checkState) => {
  const inCheck = checkState ? "True" : "False";
  document.querySelector(".check").textContent = inCheck;
};
const updateCheckmateIndicator = (checkmateState) => {
  const isCheckmate = checkmateState ? "Checkmate" : "False";
  document.querySelector(".checkmate").textContent = isCheckmate;
};
